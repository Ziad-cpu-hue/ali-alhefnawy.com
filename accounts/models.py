...
class Student(models.Model):
    ...
    parent_phone_number = models.CharField(max_length=15, unique=True, verbose_name="رقم هاتف ولي الأمر")
    ...
    def clean(self):
        """ تحقق من عدم تكرار رقم هاتف ولي الأمر """
        if self.parent_phone_number:
            qs = Student.objects.filter(parent_phone_number=self.parent_phone_number)
            if self.pk:
                qs = qs.exclude(pk=self.pk)
            if qs.exists():
                raise ValidationError({'parent_phone_number': 'رقم هاتف ولي الأمر مستخدم بالفعل في النظام.'})
